...
class Line(no_mans_sky_base_builder.part.Part):
    ...
    def deserialise_from_data(cls, data, builder_object, build_rigs=True, compensate_normal=True):
        ...
        # if not build_rigs:
        part.position_matrix(data, part, compensate_normal)
    ...
    @staticmethod
    def position_matrix(data, bpy_object, compensate_normal=True):
        # Get location data.
        pos = data.get("Position", [0.0, 0.0, 0.0])
        up = data.get("Up", [0.0, 0.0, 0.0])
        at = data.get("At", [0.0, 0.0, 0.0])
        # Set part position.
        world_matrix = Line.create_matrix_from_vectors(pos, up, at, compensate_normal=compensate_normal)
    ...
    def create_matrix_from_vectors(pos, up, at, compensate_normal=True):
        ...
        # Since Frontiers update we need to subtract the normalized at vector
        # the at vector so the wire legnth displays properly.
