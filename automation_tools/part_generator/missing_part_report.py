...
def list_all_models_in_blender():
    ...
    unreal_dir = (
        "G:/Unreal Projects/NoMansSkyBaseBuilder/Content/NMSBaseBuilder/Features/Models"
    )
...
def print_report():
    ...
    if known_parts:
        print_with_color(
            "Parts Ready To Process: {0}".format(len(known_parts)), bcolors.OKGREEN
        )
        longest_known_part = max([len(x) for x in known_parts])
        print_with_color(
            "\n".join(
                [
                    "\t" + part.rjust(longest_known_part) + " : " + known_parts[part]
    ...
    if unknown_parts:
        print_with_color(
            "Parts NOT Ready To Process (please update the DT_PartTable): {0}".format(
                len(unknown_parts)
            ),
            bcolors.FAIL,
        )
